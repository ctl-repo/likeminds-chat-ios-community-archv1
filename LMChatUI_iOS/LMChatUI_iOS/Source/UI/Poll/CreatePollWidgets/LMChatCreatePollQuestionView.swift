...
public protocol LMChatCreatePollQuestionViewProtocol: AnyObject {
    func onCrossButtonTapped(for id: Int)
    func textValueChanged(for id: Int, newValue: String?)
    func onAddNewOptionTapped()
}

open class LMChatCreatePollQuestionView: LMView {
    // MARK: UI Elements
    ...
    public weak var delegate: LMChatCreatePollQuestionViewProtocol?
    public var data: [LMChatCreatePollOptionWidget.ContentModel] = []
    ...
    // MARK: configure
    open func configure(with data: [LMChatCreatePollOptionWidget.ContentModel], delegate: LMChatCreatePollQuestionViewProtocol?) {
        self.delegate = delegate
        updateOptions(with: data)
    }
    
    open func updateOptions(with data: [LMChatCreatePollOptionWidget.ContentModel]) {
        self.data = data
    ...
    open func retrieveTextFromOptions() -> [String?] {
        optionStack.visibleCells.map { cell in
            (cell as? LMChatCreatePollOptionWidget)?.retriveText()
        }
    }
}

extension LMChatCreatePollQuestionView: UITableViewDataSource, UITableViewDelegate {
