...
open class LMChatAudioContentView: LMChatMessageContentView {
    ...
    open override func setupLayouts() {
        super.setupLayouts()
        cancelRetryContainerStackView.centerXAnchor.constraint(
            equalTo: audioPreviewContainerStackView.centerXAnchor
        ).isActive = true
        cancelRetryContainerStackView.centerYAnchor.constraint(
            equalTo: audioPreviewContainerStackView.centerYAnchor
        ).isActive = true
    }

    open override func setDataView(
        _ data: LMChatMessageCell.ContentModel, index: IndexPath
    ) {
        ...
        audioPreviewContainerStackView.bringSubviewToFront(
            cancelRetryContainerStackView)
        bubbleView.layoutIfNeeded()
    }

    func attachmentView(
        _ data: LMChatMessageCell.ContentModel, delegate: LMChatAudioProtocol?,
        index: IndexPath
    ) {
        ...
        let updatedAttachments =
            data.message.isShowMore == true
            ? attachments : Array(attachments.prefix(2))
        ...
        case .voiceNote:
            voiceNotePreview(
                updatedAttachments, delegate: delegate, index: index)
        default:
            break
        }
            ...
            attachments.count > 2
        {
            ...
            button.addTarget(
                self, action: #selector(didTapShowMore), for: .touchUpInside)
            button.setFont(Appearance.shared.fonts.buttonFont1)
            button.setTitleColor(
                Appearance.shared.colors.linkColor, for: .normal)
    ...
    func audioPreview(
        _ attachments: [AttachmentViewData], delegate: LMChatAudioProtocol?,
        index: IndexPath
    ) {
        ...
        attachments.forEach { attachment in
            ...
            preview.configure(
                with: .init(
                    fileName: attachment.name, url: attachment.url,
                    duration: attachment.meta?.duration ?? 0,
                    thumbnail: attachment.thumbnailUrl), delegate: delegate,
                index: index)
            preview.widthAnchor.constraint(equalToConstant: widthViewSize)
                .isActive = true
    ...
    func voiceNotePreview(
        _ attachments: [AttachmentViewData],
        delegate: LMChatAudioProtocol?, index: IndexPath
    ) {
        guard !attachments.isEmpty else {
            audioPreviewContainerStackView.isHidden = true
            return
        ...
        attachments.forEach { attachment in
            audioPreviewContainerStackView.addArrangedSubview(
                createAudioPreview(
                    with: .init(
                        fileName: attachment.name, url: attachment.url,
                        duration: attachment.meta?.duration ?? 0,
    ...
    func createAudioPreview(
        with data: LMChatAudioContentModel, delegate: LMChatAudioProtocol?,
        index: IndexPath
    ) -> LMChatVoiceNotePreview {
        let preview = LMUIComponents.shared.voiceNotePreview.init()
...
extension LMChatAudioContentView {
    ...
    public func seekSlider(to position: Float, url: String) {
        audioPreviewContainerStackView.subviews.forEach { sub in
            (sub as? LMChatVoiceNotePreview)?.updateSeekerValue(
                with: position, for: url)
            (sub as? LMChatAudioPreview)?.updateSeekerValue(
                with: position, for: url)
        }
    }
}
