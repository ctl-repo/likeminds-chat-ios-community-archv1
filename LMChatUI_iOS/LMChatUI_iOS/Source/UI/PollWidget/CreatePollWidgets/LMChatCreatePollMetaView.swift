...
public protocol LMChatCreatePollMetaViewProtocol: AnyObject {
    func onTapUserMetaOptions()
    func onValueChanged(for id: Int)
}

open class LMChatCreatePollMetaView: LMView {
    public struct ContentModel {
        public let metaOptions: [LMChatCreatePollMetaOptionWidget.ContentModel]
        public let optionState: String
        public let optionCount: Int
        
        public init(metaOptions: [LMChatCreatePollMetaOptionWidget.ContentModel], optionState: String, optionCount: Int) {
    ...
    open private(set) lazy var optionTypeButton: LMChatCreatePollUserOptionWidget = {
        let view = LMChatCreatePollUserOptionWidget().translatesAutoresizingMaskIntoConstraints()
        return view
    }()
    
    open private(set) lazy var maxOptionButton: LMChatCreatePollUserOptionWidget = {
        let view = LMChatCreatePollUserOptionWidget().translatesAutoresizingMaskIntoConstraints()
        return view
    }()
    ...
    public weak var delegate: LMChatCreatePollMetaViewProtocol?
    ...
    // MARK: configure
    open func configure(with data: ContentModel, delegate: LMChatCreatePollMetaViewProtocol?) {
        self.delegate = delegate
        
        data.metaOptions.forEach { option in
            let view = LMChatCreatePollMetaOptionWidget().translatesAutoresizingMaskIntoConstraints()
