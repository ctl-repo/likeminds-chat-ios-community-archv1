            ...
            for attachment in attachments {
                group.addTask {
                    return try await Task.detached(priority: .background) {
                        ...
                        do {
                            let awsFilePath = try await LMChatAWSManager.shared.uploadFileAsync(
                                fileUrl: fileUrl,
                            ...
                            if let thumbFileUrl = URL(string: attachment.thumbnailLocalFilePath ?? ""),
                               let thumbnailAWSFolderPath = attachment.thumbnailAWSFolderPath {
                                ...
                                awsThumbnailFilePath = try await LMChatAWSManager.shared.uploadFileAsync(
                                    fileUrl: thumbFileUrl,
                                    awsPath: thumbnailAWSFolderPath,
                                    fileName: "\(thumbFileUrl.pathExtension)",
                                    contentType: "image",
                                    withTaskGroupId: "\(thumbFileUrl.pathExtension)"
                                )
                            }
