...
open class LMChatDMFeedViewController: LMViewController {
    ...
    open func setupNewFabButton() {
        fabButtonWidthConstraints = newDMFabButton.widthAnchor.constraint(
            equalToConstant: 120)
        fabButtonWidthConstraints?.isActive = true
        newDMFabButton.setInsets(
            forContentPadding: UIEdgeInsets(
                top: 5, left: 5, bottom: 5, right: 5), imageTitlePadding: 10)
    ...
    @objc open func newFabButtonClicked() {
        if let delegate, let communityId = viewModel?.getCommunityId() {
            ...
            NavigationScreen.shared.perform(
                .dmMemberList(showList: viewModel?.showList), from: self,
                params: nil)
        }
    }
