...
public final class LMChatDMCreationHandler {
    ...
    public func openDMChatroom(
        uuid: String, viewController: LMViewController?,
        completion: ((_ chatroomId: String?) -> Void)?
    ) {
        self.viewController = viewController
        self.completion = completion
        (self.viewController)?.showHideLoaderView(
            isShow: true, backgroundColor: .clear)
        self.checkDMLimit(uuid: uuid)
    }

    public func validateDMChatroom(
        withUUID uuid: String, completion: ((_ chatroomId: String?) -> Void)?
    ) {
        self.completion = completion
        self.checkDMLimit(uuid: uuid)
    }
    ...
    private func checkDMLimit(uuid: String) {
        let request = CheckDMLimitRequest.builder()
            .uuid(uuid)
            .build()
        LMChatClient.shared.checkDMLimit(request: request) {
            [weak self] response in
            (self?.viewController)?.showHideLoaderView(
                isShow: false, backgroundColor: .clear)
            guard let chatroomId = response.data?.chatroomId else {
                if LMSharedPreferences.bool(
                    forKey: LMSharedPreferencesKeys.isDMWithRequestEnabled
                        .rawValue) == true
                {
                    if response.data?.isRequestDMLimitExceeded == false {
                        ...
                        var message =
                            "You can send only \(response.data?.userDMLimit?.numberInDuration ?? 0) DM requests per \(response.data?.userDMLimit?.duration ?? "Day")."
                        ...
                        if let duration = response.data?.newRequestDMTimestamp {
                            ...
                            let diff = dateComponentsFormatter.localizedString(
                                for: date, relativeTo: Date())
                            message.append("\nTry again \(diff).")
                        }

                        self?.viewController?.showErrorAlert(
                            "Request limit exceeded", message: message)
                        self?.completion?(nil)
                    }
                } else {
                    self?.createDMChatroom(uuid: uuid)
    ...
    private func routeToChatroom(chatroomId: String?) {
        ...
        DispatchQueue.main.async {
            NavigationScreen.shared.perform(
                ...
                from: viewController, params: nil)
        }
    }
}
